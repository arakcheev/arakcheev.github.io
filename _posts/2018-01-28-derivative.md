...
> Производная $f^{'}(x_0)$ существует тогда и только тогда, когда $f^{'}_{+}(x_0) = f^{'}_{-}(x_0)$.
...
Обратное неверно, так как есть много примеров непрерывных функций в нуле, например $f(x) = |x|$, но производной не существует (односторонние производные разные).
